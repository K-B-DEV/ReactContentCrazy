...
const MovieDetail = () => {
  ...
  const getStars = () => {
    const stars = [];
    const roundedRating = Math.floor(movie_details.vote_average * 2) / 2;
    for (let i = 1; i <= 10; i++) {
      if (roundedRating - i > 1) {
        ...
        stars.push(
          ...
          //   key={i}
          //   src={FullStar}
          //   title={movie_details.vote_average}
          // ></img>
          <Star key={i} />
        );
      } else {
  ...
  const imgPath = `https://image.tmdb.org/t/p/w1280/${movie_details.backdrop_path}`;
  const click = () => {};
  const nPathId = parseInt(pathId);
  console.log("Detail: " + nPathId);
  console.log(typeof nPathId);
  return (
    <>
      <StyledBackgroundContainer
        ...
        exit="exit"
      >
        <StyledDetailView layoutId={nPathId}>
          <StyledTvTitle>
            <h2>{movie_details.original_title}</h2>
          </StyledTvTitle>
          <StyledInline>
            <StyledTvRating>
              <div>{getStars()}</div>
            ...
            <StyledSeasonsEpisodes>
              ...
              <StyledSeasons>{movie_details.runtime} minutes</StyledSeasons>
            </StyledSeasonsEpisodes>
          </StyledInline>
          <StyledImage src={imgPath} alt={movie_details.name} />
          <Accordion
          ...
          >
            ...
            <AccordionDetails>
              <Typography>
                {movie_details.overview !== "" ? movie_details.overview : "N/A"}
              </Typography>
            </AccordionDetails>
          </Accordion>
...
const StyledBackgroundContainer = styled(motion.div)`
